...
import { getFirestore, doc, getDoc, collection, addDoc, getDocs, setDoc } from 'firebase/firestore';
...
const documentIds = [
    '7tA6ZDPCfwWJyX6WR3fa',
	'ZxrMPTyRh1JIJKyOaMML',    
	'blktITROSV5r9pHN5EJ3',
    'fLebkeid1dwpd5qtqEYA',
    'ofCXaDHdAvIwOlMtA7fL',
    'skY2fkBjjqCymvVuc3me'
];
...
export const getDashboardSections = async (): Promise<DashboardSectionItem[]> => {
    try {
        const { db } = await getFirebaseInstance();
        const data: DashboardSectionItem[] = [];

        for (const id of documentIds) {
            const docRef = doc(db, 'dashboard_sections', id);
            const docSnap = await getDoc(docRef);
            
            if (docSnap.exists()) {
                const docData = docSnap.data();
                // Asegurarse de que img y description existan; si falta alguno, se asigna un valor predeterminado
                data.push({
                    img: docData.img || 'default-image-url.jpg', // Usa una URL de imagen predeterminada si falta
                    description: docData.description || 'Descripción no disponible'
                });
            } else {
